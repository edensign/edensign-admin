...
const initialValues = { name: "", state_id: "" };

const CityFormComponent = ({ openDialog, setOpenDialog }) => {
    ...
    useEffect(() => {
        ...
        if (id) {
            setTitle("Update");
            setLoading(true);
            API.CommonAPI.multipleAPICall("GET", [`/get-by-pk/city/${id}`])
                .then(res => {
                    if (res[0].data.status === "Success") {
                        const cityData = res[0].data.data;
                        setInitialState(cityData);
                        // Fetch the state details to get the country_id
                        if (cityData.state_id) {
                            API.CommonAPI.multipleAPICall("GET", [`/get-by-pk/state/${cityData.state_id}`])
                                .then(stRes => {
                                    if (stRes[0].data.status === "Success") {
                                        const stateData = stRes[0].data.data;
                                        setSelectedCountry(stateData.country_id);
                                        // Now fetch all states for that country
                                        API.StateAPI.getStates(stateData.country_id).then(allStRes => {
                                            if (allStRes?.status === "Success") setStates(allStRes.data.list);
                                        });
                                    }
                                });
                        }
    ...
    const handleCountryChange = (countryId) => {
        ...
        setStates([]);
        if (countryId) {
            API.StateAPI.getStates(countryId).then(res => {
                if (res?.status === "Success") setStates(res.data.list);
            });
        }
    };
    ...
    const handleSubmit = (values) => {
        ...
        const action = values.id
            ? API.CityAPI.updateCity(values)
            : API.CityAPI.createCity(values);
    ...
    return (
        <div>
            <Dialog
                ...
                onClose={handleClose}
                sx={{
                    top: isMobile ? "25%" : isTab ? "15%" : "10%",
                    height: isMobile ? "60%" : isTab ? "60%" : "80%",
                        ...
                        <form onSubmit={handleSubmit}>
                            <Box display="grid" gap="30px" gridTemplateColumns="repeat(2, minmax(0, 1fr))" padding="20px">
                                ...
                                <FormControl variant="filled">
                                    ...
                                    >
                                        <MenuItem value=""><em>Select Country first</em></MenuItem>
                                        {countries.map(c => (
