...
const FormComponent = ({ openDialog, setOpenDialog }) => {
    ...
    const updateSkill = (values) => {
        setLoading(true);
        API.SkillAPI.updateSkill(values)
            .then(({ data: skill }) => {
                if (skill?.status === 'Success') {
                    ...
                    setTimeout(() => {
                        handleDialogClose();
                        location.href = "/skill/listing";     //same as location.reload()
                    }, 2000);
    ...
    const createSkill = (values) => {
        setLoading(true);
        API.SkillAPI.createSkill(values)
            .then(({ data: skill }) => {
                if (skill?.status === 'Success') {
                    setLoading(false);
        ...
        <div>
            ...
            >
                ...
                <Formik
                    ...
                    onSubmit={values => {
                        values.id ? updateSkill(values) : createSkill(values);
                    }}
                >
                    {({
                        values,
