...
const FormComponent = ({ openDialog, setOpenDialog }) => {
    ...
    const updateService = (values) => {
        setLoading(true);
        API.ServiceAPI.updateService(values)
            .then(({ data: service }) => {
    ...
    const populateData = (id) => {
        setLoading(true);
        const path = [`/get-by-pk/service/${id}`];
    ...
    const createService = (values) => {
        setLoading(true);
        API.ServiceAPI.createService(values)
            .then(({ data: service }) => {
                if (service?.status === 'Success') {
                    setLoading(false);
        ...
        <div>
            ...
            >
                ...
                <Formik
                    ...
                    onSubmit={values => {
                        values.id ? updateService(values) : createService(values);
                    }}
                >
                    {({
                        values,
                        ...
                        <form onSubmit={handleSubmit}>
                            ...
                            <Box display="flex" justifyContent="end" p="20px">
                                {   //hide reset button on service update
                                    title === "Update" ? null :
